    ...
    const contentObserver = new MutationObserver(mutations => {
        ...
        if (battleSummary) {
            ...
            if (eval(parseFor(battleSummary, "Exp").split("- ")[1].split("*")[0].replace(/,/g, "")) > 0) {
                ...
                let finalLevel = 1 / 50 * (Math.sqrt(4 * Math.pow(expPerAction, 2) - 100 * expPerAction * (2 * currentLevel + 1) + 25 * (8 * bankedExperience + 25 * Math.pow(2 * currentLevel + 1, 2))) + 2 * expPerAction - 25);
